...
async function checkRentalsValues() {
  try {
    ...
    const { data: rentals, error } = await supabase
      .from('rentals')
      .select('id, title, make, model, body_type, fuel_type, transmission, daily_rate, location, rental_type, minimum_days, maximum_days, delivery_available, insurance_included')
      .eq('is_active', true)
...
('BMW X3 - Luxury SUV', 'BMW', 'X3', 2023, 12000, 'Colombo', 'Petrol', 'Automatic', 'SUV', 'daily', '00000000-0000-0000-0000-000000000000');
    ...
    rentals.slice(0, 3).forEach((rental, index) => {
      ...
      console.log(`   Rental Type: "${rental.rental_type}"`);
    ...
    const uniqueRentalTypes = [...new Set(rentals.map(r => r.rental_type).filter(Boolean))].sort();
    ...
    console.log('RENTAL_TYPE values:', JSON.stringify(uniqueRentalTypes, null, 2));
    ...
    if (uniqueRentalTypes.length > 0) {
      console.log('For RENTAL_TYPE filter options, use these values:');
      uniqueRentalTypes.forEach(rentalType => {
        console.log(`  { value: '${rentalType}', label: '${rentalType}', count: ${rentals.filter(r => r.rental_type === rentalType).length} },`);
      });
      console.log('');
    }
